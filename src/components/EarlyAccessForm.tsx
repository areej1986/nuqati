...
import { saveRegistration } from '../services/sheetsService';
...
export const EarlyAccessForm: React.FC = () => {
  ...
  const [submittedRecord, setSubmittedRecord] =
    useState<SignupRecord | null>(null);
  ...
  // قراءة حالة التسجيل المحفوظة محليًا في المتصفح
  useEffect(() => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(
      '[Noqati 1/5] تم الضغط على زر التسجيل واستدعاء handleSubmit.',
      'البيانات:',
      formData,
      'الفئات العمرية:',
      selectedAges
    ...
    // التحقق من وسيلة التواصل
    if (!formData.contact.trim()) {
      setErrorMessage(
        'يرجى إدخال رقم الواتساب أو البريد الإلكتروني للتواصل معك عند الإطلاق.'
      );
    ...
    // التحقق الأساسي من صيغة البريد أو الهاتف
    const isEmail = formData.contact.includes('@');

    const isPhone = /^[0-9+()\s-]{7,18}$/.test(
      formData.contact.trim()
    );

    if (!isEmail && !isPhone) {
      setErrorMessage(
        'يرجى إدخال رقم هاتف واتساب صحيح أو بريد إلكتروني صالح.'
      );

      console.warn(
        '[Noqati 1/5] صيغة وسيلة التواصل غير صحيحة:',
        formData.contact
      );
    ...
    if (selectedAges.length === 0) {
      setErrorMessage(
        'يرجى اختيار فئة عمرية واحدة على الأقل لأطفالك.'
      );

      return;
    }

    // التحقق من المشكلة المخصصة
    if (
      formData.mainChallenge === 'مشكلة أخرى' &&
      !formData.customChallenge?.trim()
    ) {
      setErrorMessage(
        'يرجى كتابة المشكلة التي تواجهينها في الخانة المخصصة.'
      );
    ...
    try {
      // حفظ التسجيل مباشرة في Supabase
      await saveRegistration(submissionData);

      console.log(
        '[Noqati] اكتملت عملية التسجيل بنجاح!'
      );
      ...
      // حفظ حالة النجاح محليًا في المتصفح فقط
      // هذا لا يقوم بقراءة أي بيانات من Supabase
      try {
      ...
      console.error(
        '[Noqati خطأ] فشل أثناء حفظ التسجيل:',
        saveError
      );
      ...
      setErrorMessage(
        errMsg ||
          'تعذر إكمال التسجيل حالياً، يرجى المحاولة مرة أخرى.'
      );
    } finally {
  ...
  const handleCopyLink = async () => {
    try {
      ...
      setErrorMessage(
        'تعذر نسخ الرابط. يمكنك نسخه يدوياً.'
      );
    }
  };
  ...
  return (
    <section
      id="early-access"
      className="py-20 bg-gradient-to-b from-stone-50 via-orange-50/30 to-amber-50/40 relative"
    >
            ...
            {!submittedRecord ? (
              ...
              >
                ...
                <div className="text-center max-w-2xl mx-auto mb-10">
                  ...
                  <p className="text-stone-600 text-sm sm:text-base leading-relaxed">
                    انضمي لقائمة الانتظار لتكوني من أوائل
                    العائلات التي تجرب تطبيق "نقاطي"، مع{' '}
                    <strong>
                      اشتراك تجريبي VIP مجاناً لمدة ٣ أشهر
                    </strong>{' '}
                    عند الإطلاق.
                  </p>
                </div>
                ...
                >
                  ...
                  <div>
                    ...
                    >
                      رقم الواتساب أو البريد الإلكتروني{' '}
                    ...
                    <span className="text-[11px] text-stone-500 mt-1 block">
                      لن نرسل أي رسائل مزعجة؛ سنخبرك فور توفر
                      التطبيق للتجربة.
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {AGE_CATEGORIES.map((category) => {
                        const isChecked =
                          selectedAges.includes(category);
                  ...
                  {/* Main Parenting Challenge */}
                  <div>
                    <label
                      htmlFor="challenge-select"
                      className="block text-sm font-bold text-stone-900 mb-2"
                    >
                      أكبر تحدٍ يواجهك في روتين أطفالك
                      اليومي؟ (اختياري)
                    </label>
                    ...
                    >
                      <option value="المماطلة وتأجيل الواجبات">
                        المماطلة وتأجيل الواجبات والمذاكرة
                      </option>

                      <option value="ترتيب الغرفة والألعاب">
                        عدم ترتيب الغرفة والألعاب إلا بعد صراخ
                      </option>

                      <option value="كثرة وقت الشاشات">
                        صعوبة سحب الأجهزة اللوحية والشاشات
                      </option>

                      <option value="العناد والجدال المستمر">
                        العناد والجدال عند طلب أي مهمة روتينية
                      </option>

                      <option value="النظافة والنوم المبكر">
                        تأخير النوم وروتين النظافة وتفريش الأسنان
                      </option>

                      <option value="مشكلة أخرى">
                        مشكلة أخرى
                      </option>
                    </select>
                    ...
                    {formData.mainChallenge ===
                      ...
                      <div className="mt-3">
                        <label
                          htmlFor="custom-challenge-input"
                  ...
                  <div className="flex items-center justify-center gap-2 text-xs text-stone-500 text-center">
                    ...
                    <span>
                      خصوصيتك مضمونة ١٠٠٪ ولن نشارك بياناتك
                      مع أي طرف ثالث.
                    </span>
                  </div>
                </form>
              ...
              >
                ...
                <p className="text-stone-600 text-sm sm:text-base leading-relaxed mb-6">
                  شكرًا لانضمامك إلى نقاطي. سنخبرك فور توفر
                  التطبيق للتجربة.
                </p>
                ...
                <div className="p-5 rounded-2xl bg-orange-50 border border-orange-200 text-right mb-8">
                  ...
                  <ul className="text-xs text-stone-700 space-y-1.5">
                    <li className="flex items-center gap-1.5">
                      ...
                      <span>
                        تجربة مجانية كاملة ومميزة (VIP) لمدة
                        ٣ أشهر.
                      </span>
                    </li>
                    ...
                    <li className="flex items-center gap-1.5">
                      ...
                      <span>
                        قوالب مهام وجداول تربوية حصرية قابلة
                        للطباعة فورياً.
                      </span>
                    </li>
                    ...
                    <li className="flex items-center gap-1.5">
                      ...
                      <span>
                        أولوية التواصل مع فريق التطوير لطلب أي
                        ميزة تفضلينها.
                      </span>
                    </li>
                  </ul>
                ...
                <div className="space-y-3">
                  <span className="text-xs font-bold text-stone-700 block">
                    شاركي الفكرة مع أمهات وصديقات يعانين من نفس
                    التحدي:
                  </span>
