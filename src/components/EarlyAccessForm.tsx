...
export const EarlyAccessForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Send registration to Google Apps Script Web App
      const result = await saveRegistrationToGoogleSheets(submissionData);
      console.log('[Noqati] اكتملت عملية التسجيل بنجاح!');
      ...
      console.error('[Noqati خطأ] فشل أثناء إرسال النموذج:', saveError);
      const errMsg = saveError instanceof Error ? saveError.message : String(saveError);
      setErrorMessage(errMsg || 'تعذر إكمال التسجيل حالياً، يرجى المحاولة مرة أخرى.');
    } finally {
